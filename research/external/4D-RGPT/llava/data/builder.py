...
def register_datasets(name: Optional[str] = None):
    if name is None:
        name = os.environ.get("VILA_DATASETS", "cs-oci-ord")
...
def register_mixtures():
    yamlpath = os.environ.get(
        "VILA_MIXTURES", 
        os.path.join(os.path.dirname(__file__), "registry", "mixtures.yaml"))
    logger.info(f"Registering mixtures from environment: '{yamlpath}'.")
    return io.load(yamlpath)
