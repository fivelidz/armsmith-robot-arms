...
# Matches NVILA-Lite-8B's tokenizer chat_template (Qwen2 ChatML). Used by the
# lmms-eval `vila` adapter, which calls Conversation.get_prompt() directly and
# can't go through tokenizer.apply_chat_template — so we render ChatML by hand.
qwen_2 = Conversation(
