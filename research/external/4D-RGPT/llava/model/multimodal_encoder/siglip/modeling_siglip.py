...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor, interpolate_pos_encoding=False) -> torch.Tensor:
        ...
        if interpolate_pos_encoding:
            ...
            # transformers >=5.0 uses meta-tensor materialization in
            # from_pretrained and skips non-persistent buffers set in __init__,
            # leaving `self.position_ids` filled with uninitialized memory.
            # The garbage indices land OOB of `position_embedding`'s
            # num_positions-sized table → CUDA `vectorized_gather_kernel index
            # out of bounds` on the first forward (varies by memory layout, so
            # the crash is non-deterministic). Rebuild the index range inline
            # so we never read the buffer. Observationally identical on v4
            # since the buffer there *was* arange(num_positions) at init time.
            position_ids = torch.arange(
                self.num_positions, device=embeddings.device
            ).unsqueeze(0)
...
class SiglipPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, SiglipVisionEmbeddings):
            ...
            # transformers v5's smart_apply walks every submodule, including
            # some with <2-D weights (e.g. MHA internals after re-init).
            # lecun_normal_ -> variance_scaling_ requires fan_in/fan_out,
            # which is undefined for <2-D tensors.
            if module.weight.dim() >= 2:
