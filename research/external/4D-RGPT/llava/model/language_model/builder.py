...
def build_llm_and_tokenizer(
    ...
    **kwargs,
    ...
    # DeepSpeed-style checkpoint saves drop the slow tokenizer files (vocab.json,
    # merges.txt) and only keep tokenizer.json. Prefer slow to preserve legacy
    # behavior on full HF saves; fall back to fast when only the fast file is
    # available. Qwen2 fast and slow tokenizers produce identical token IDs.
    try:
        tokenizer = AutoTokenizer.from_pretrained(llm_path, padding_side="right", use_fast=False, legacy=False)
    except (TypeError, OSError):
        tokenizer = AutoTokenizer.from_pretrained(llm_path, padding_side="right", use_fast=True)
    if model_max_length is not None:
    ...
    # NOTE: 4D_RVILA
    # from tools.find_tokens import find_tokens
    # find_tokens(tokenizer)
    # raise
