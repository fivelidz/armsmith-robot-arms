...
from llava.model.low_level_perception.builder import build_l4p_encoder_decoder, build_distillation
from llava.model.pe.time_pe import TimePositionalEncoding
from llava.model.region_extractor.builder import build_region_extractor, RegionExtractor
...
class LlavaMetaModel(ABC):
    def init_vlm(self, config, *args, **kwargs):
        ...
        cfgs = get_model_config(
            config,
            default_keys=["llm_cfg", "vision_tower_cfg", "mm_projector_cfg", "region_extractor_cfg"])
        if len(cfgs) >= 3:
            llm_cfg, vision_tower_cfg, mm_projector_cfg = cfgs[:3]
        else:
        ...
        # NOTE: 4D_RVILA
        region_extractor_cfg = cfgs[3] if len(cfgs) > 3 else None

        num_patches = self.get_vision_tower().num_patches
        downsample_rate = self.get_mm_projector().downsample_rate
        self.num_image_tokens = math.ceil(num_patches**0.5 / downsample_rate) ** 2

        self.encoder_l4p = build_l4p_encoder_decoder("", config)
        # self.low_level_heads = build_low_level_heads()
        self.distillation = build_distillation("", config)

        # 4D-RGPT Timestamp Positional Encoding (paper §3, eq. 5). Sinusoidal,
        # no learned params. Added to vision features before the mm_projector
        # in encode_images_time_region.
        if getattr(config, "use_time_pe", False):
            self.time_pe = TimePositionalEncoding(d_model=config.mm_hidden_size)
    ...
    #     # NOTE: 4D_RVILA
    #     vlm.encoder_l4p, vlm.decoder_l4p = build_l4p_encoder_decoder()
    #     vlm.low_level_heads = build_low_level_heads()
    ...
    def get_time_pe(self):
        return getattr(self, "time_pe", None)

    def get_region_extractor(self) -> RegionExtractor | None:
    ...
    def post_config(self):
        ...
        if getattr(self.config, "mm_projector_cfg", None) is None:
            self.config.mm_projector_cfg = self.mm_projector.config
        if getattr(self.config, "pe_cfg", None) is None and self.get_pe() is not None:
            self.config.pe_cfg = self.pe.config
        if getattr(self.config, "region_extractor_cfg", None) is None and self.get_region_extractor() is not None:
            self.config.region_extractor_cfg = self.region_extractor.config
    ...
    def freezed_module_patch(self):
        ...
        if self.training:
            ...
            if self.get_pe() and not getattr(self.config, "tune_pe", False):
                self.get_pe().eval()
    ...
    def encode_images_time_region(
        self,
        images,
        sampled_sec,
        # masks,
        # block_sizes: Optional[Optional[Tuple[int, ...]]] = None,
        block_lengths: Optional[Optional[Tuple[int, ...]]] = None,
        # add_pe: Optional[Optional[Tuple[int, ...]]] = None,
    ) -> List[torch.Tensor]:
        logger.debug("[encode_images_time_region] called.")
        # torch.distributed.breakpoint()
        image_features = self.get_vision_tower()(images)

        # 4D-RGPT TPE: add sinusoidal time encoding per frame, before the projector.
        # Cast pe to image_features.dtype: TimePositionalEncoding builds the encoding
        # in float32 (its torch.zeros default + float32 div_term), so without the cast
        # the bf16 image_features would silently promote to fp32 and cascade through
        # the projector + LLM, breaking Z3+autocast assumptions.
        time_pe = self.get_time_pe()
        if time_pe is not None and sampled_sec is not None:
            t = torch.tensor(
                list(chain.from_iterable(sampled_sec)),
                device=image_features.device, dtype=image_features.dtype,
            )  # (Σframes,)
            pe = time_pe(t.unsqueeze(0)).squeeze(0).to(image_features.dtype)  # (Σframes, mm_hidden)
            image_features = image_features + pe.unsqueeze(1)  # broadcast over tokens
        # hres_tower_features, lres_tower_features = self.get_region_extractor().feature_refinement(image_features)
        # hres_features_dim = int(hres_tower_features.size(1) ** 0.5)
        # lres_features_dim = int(lres_tower_features.size(1) ** 0.5)

        # xyzs = xyzs.permute(0, 3, 1, 2)  # Move channel to second dimension
        # hres_xyzs = torch.nn.functional.interpolate(xyzs, size=(hres_features_dim, hres_features_dim), mode="area")
        # lres_xyzs = torch.nn.functional.interpolate(xyzs, size=(lres_features_dim, lres_features_dim), mode="area")
        # del xyzs  # Free memory
        # torch.cuda.empty_cache()

        # hres_flattened_xyzs = rearrange(hres_xyzs.permute(0, 2, 3, 1), "b h w c -> b (h w) c")
        # lres_flattened_xyzs = rearrange(lres_xyzs.permute(0, 2, 3, 1), "b h w c -> b (h w) c")

        # # memory efficient forwarding, but currently has multi-node training issue
        # # hres_xyz_features = self.get_pe().mask_forward(hres_flattened_xyzs, masks)

        # hres_xyz_features = self.get_pe()(hres_flattened_xyzs)
        # lres_xyz_features = self.get_pe()(lres_flattened_xyzs)

        # if self.config.usepe:
        #     hres_tower_features = hres_tower_features + hres_xyz_features

        # assert len(images) == len(masks), f"images: {len(images)}, masks: {len(masks)}"
        # mask_features, dummy_mask_features = self.get_region_extractor()(hres_tower_features, masks, block_lengths)

        # if self.config.usepe:
        #     lres_tower_features = lres_tower_features + lres_xyz_features

        # lres_tower_features = self.get_mm_projector()(lres_tower_features)
        image_features = self.get_mm_projector()(image_features)

        return image_features

    def encode_images_w_xyzs(
        self,
        images,
        xyzs,
        masks,
        ...
        return lres_tower_features, mask_features, dummy_mask_features
...
class LlavaMetaForCausalLM(ABC):
    ...
    def __embed_media_tokens(
        ...
        for name in media:
            ...
            if "mask" in media:
                logger.info("mask")
            if name == "video" and "sampled_sec" in media:
                logger.debug("Use time embed")
                sampled_sec = media.pop("sampled_sec", None)
                embeds[name] = deque(self.encoders[name](media[name], media_config[name], sampled_sec))
            else:
    ...
    def prepare_inputs_for_generation(self, *args, **kwargs):
        # PEFT's PeftModelForCausalLM.__init__ snapshots this attribute off the
        # base model (peft/peft_model.py: `self.base_model_prepare_inputs_for_
        # generation = self.base_model.prepare_inputs_for_generation`) — without
        # it, get_peft_model(LlavaLlamaModel, ...) raises AttributeError.
        # Our `generate()` override (below) bypasses PEFT's generation path and
        # calls `self.llm.generate(inputs_embeds=...)` directly, so this method
        # is just a satisfying-the-API delegate; in practice it isn't called.
    ...
    def generate_content(self, prompt: Union[str, List], generation_config: Optional[GenerationConfig] = None) -> str:
        ...
        from llava.utils.logging import logger
        #logger.info(media.keys())

        # NOTE: 4D_RVILA
        if hasattr(self.config, "svila") and self.config.svila:
            if "image" in media and "depth" not in media and self.config.svila:
                import PIL

                for pil_image in media["image"]:
                    if pil_image.mode != "RGB":
                        pil_image = pil_image.convert("RGB")
                    depth_inputs = self.depth_processor(images=pil_image, return_tensors="pt").to(
                        self.depth_estimator.device
                    )
                    raw_depth = self.depth_estimator(**depth_inputs)
                    post_depth = self.depth_processor.post_process_depth_estimation(
                        raw_depth,
                        target_sizes=[(pil_image.height, pil_image.width)],
                    )[0]["predicted_depth"]

                    normed_depth = (1 - (post_depth - post_depth.min()) / (post_depth.max() - post_depth.min())) * 255.0
                    normed_depth = normed_depth.cpu().numpy().astype(np.uint8)
                    media["depth"].append(PIL.Image.fromarray(normed_depth))

        # logger.debug(media["video_frames"])
        # media.pop("video_frames")
        ...
        for name in media:
            if name == "image":
                ...
                # logger.debug(f"num images: {len(media['image'])}")
            elif name == "video":
                media[name] = [
                    process_images(images, self.vision_tower.image_processor, self.config).half()
                    for images in media[name]
                ]
            elif 'sampled_sec' in media:
                # NOTE: 4D_RVILA
                logger.debug('sampled_sec')
                # media.pop('sampled_sec')
                # logger.debug(f"video length: {len(media['video'][0])}")
            else:
        ...
        try:
            ...
            # logger.debug(f"Output shape: {output_ids.shape}")
        except ValueError:
