...
class BasicVideoEncoder(BaseEncoder):
    ...
    def forward(self, videos: List[torch.Tensor], config: Dict[str, Any], sampled_sec: Optional[torch.Tensor] = None) -> List[torch.Tensor]:
        num_frames = [video.shape[0] for video in videos]
        images = torch.cat(videos, dim=0)
        if sampled_sec is not None:
            features = self.parent.encode_images_time_region(images, sampled_sec)
        else:
