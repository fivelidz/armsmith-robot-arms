...
def prepare_config_for_training(
    ...
    # NOTE; 4D_RVILA
    if getattr(config, "pe_cfg", None) is None:
        config.pe_cfg = model_args.pe
    if getattr(config, "region_extractor_cfg", None) is None:
        config.region_extractor_cfg = model_args.region_extractor
    config.tune_pe = training_args.tune_pe
    config.tune_region_extractor = training_args.tune_region_extractor
