...
def train():
    ...
    if training_args.use_one_logger:
        from one_logger_utils.huggingface import hook_trainer_cls, TimeEventCallback
    ...
    if training_args.gradient_checkpointing:
        ...
        # Non-reentrant checkpoint's `check_recomputed_tensors_match` (torch>=2.4)
        # validates that tensor metadata matches between forward and recompute.
        # This misfires for our setup in two ways:
        #   • LoRA: PEFT swaps requires_grad on base weights between forward
        #     and recompute.
        #   • DeepSpeed ZeRO-3: params are gathered for forward then released;
        #     recompute sees the partition (shape [0]) instead of the full
        #     gathered tensor → CheckpointError with shape [N] vs [0] mismatch.
        # We use `use_reentrant=False` (required for newer transformers) AND
        # `determinism_check="none"` to no-op the validator. Both are needed:
        # the repo always uses Z3 (scripts/zero3.json), so the Z3 gather/release
        # mismatch hits regardless of LoRA. See torch/utils/checkpoint.py:1064
        # (`"none": lambda _: None`) for how this disables the check.
        if training_args.gradient_checkpointing_kwargs is None:
            training_args.gradient_checkpointing_kwargs = {}
        training_args.gradient_checkpointing_kwargs.setdefault("use_reentrant", False)
        training_args.gradient_checkpointing_kwargs.setdefault("determinism_check", "none")
    ...
    if training_args.lora_enable:
        ...
        # NOTE: 4D_RVILA
        model.distillation.requires_grad_(True)
        model.print_trainable_parameters()
    else:
        model.get_llm().requires_grad_(training_args.tune_language_model)
        ...
        if model.get_vision_tower():
            ...
            pe = model.get_pe()
            if pe:
                pe.requires_grad_(training_args.tune_pe)
                mprint(f"pe {training_args.tune_pe}")
    ...
    if vision_tower is not None:
        ...
        # The L4P teacher (encoder_l4p) was built in LlavaMetaModel.init_vlm with
        # the pretrained config's num_video_frames (often 8), baking that T into
        # `self.intrinsics_b44t`. Now that we've overridden num_video_frames,
        # rebuild L4P's intrinsics to match — otherwise rays_to_cameras inside
        # the dpt heads sees T_camray != T_intrinsics and crashes.
        # Keeping this fix on the 4D-RGPT side leaves the upstream L4P code
        # (llava/model/low_level_perception/) untouched.
        if getattr(model, "encoder_l4p", None) is not None:
            import einops as _einops
            from llava.model.low_level_perception.base import default_intrinsics
            from l4p.utils.geometry_utils import normalize_intrinsics
            l4p = model.encoder_l4p
            n_frames = model.config.num_video_frames
            intrinsics_b44 = default_intrinsics(l4p.img_size, l4p.img_size)
            l4p.intrinsics_b44t = _einops.repeat(
                intrinsics_b44, "b h w -> b h w t", t=n_frames
            ).to(l4p.intrinsics_b44t.device, dtype=l4p.intrinsics_b44t.dtype)
            l4p.intrinsics = normalize_intrinsics(
                l4p.intrinsics_b44t, l4p.img_size, l4p.img_size
            )

            # Swap the "camray" head from VideoMAETraj3DDPTHead → VideoMAECameraDPTHead.
            #
            # Why: VideoMAETraj3DDPTHead.forward calls rays_to_cameras, which
            # does in-place writes on `extrinsics_b44t` after taking a strided
            # view of it. autograd saves the view for backward; the in-place
            # write bumps the storage version → backward fails. Original L4P
            # training never backpropped through here, but 4D-RGPT's explicit
            # distillation (paper L_ED) does.
            #
            # VideoMAECameraDPTHead (commented out in upstream base.py:121-123)
            # produces the same underlying DPT outputs but RETURNS the raw 6-D
            # Plücker rays directly (shape B,6,T,H,W) instead of converting to
            # extrinsics/intrinsics. For distillation purposes the rays are a
            # cleaner, more informative target anyway (no derived geometry).
            #
            # Both classes inherit from VideoMAEBaseHead and share the same DPT
            # submodule architecture, so the pretrained weights (loaded into
            # the Traj3D head's task_head) are state-dict-compatible with the
            # Camera head's task_head.
            if "camray" in l4p.task_heads:
                from l4p.models.task_heads.dense_heads import VideoMAECameraDPTHead
                old_camray = l4p.task_heads["camray"]
                _device = next(old_camray.parameters()).device
                _dtype = next(old_camray.parameters()).dtype
                new_camray = VideoMAECameraDPTHead(
                    task_name="camray",
                    hooks_idx=[14, 21, 28, 36],
                ).to(device=_device, dtype=_dtype)
                # The DPT submodule architecture matches; transfer its weights.
                new_camray.task_head.load_state_dict(old_camray.task_head.state_dict())
                l4p.task_heads["camray"] = new_camray
                logger.warning(
                    "Swapped L4P 'camray' head: VideoMAETraj3DDPTHead → "
                    "VideoMAECameraDPTHead. Distillation now targets raw "
                    "Plücker rays (B,6,T,H,W) instead of derived poses, "
                    "avoiding the rays_to_cameras autograd in-place bug."
                )
        ...
        # Parse --distill_tasks (comma-separated) and stash on model.config so the
        # forward path (llava_llama.perception_distillation) can read it.
        # "" means "all tasks the encoder provides".
        distill_tasks = [t.strip() for t in (model_args.distill_tasks or "").split(",") if t.strip()]
        if distill_tasks and getattr(model, "encoder_l4p", None) is not None:
            available = set(model.encoder_l4p.tasks)
            unknown = [t for t in distill_tasks if t not in available]
            if unknown:
                raise ValueError(
                    f"--distill_tasks contains tasks not in encoder_l4p.tasks: {unknown}. "
                    f"Available: {sorted(available)}"
                )
        model.config.distill_tasks = distill_tasks
        # Stash so llava_llama.perception_distillation can write viz next to checkpoints.
        model.config.output_dir = training_args.output_dir
        ...
        logger.debug(f"num_patches: {num_patches}, downsample_rate: {downsample_rate}, num_image_tokens: {num_image_tokens}")
    ...
    if training_args.auto_eval_tasks:
        from llava.train.callbacks.lmms_eval_callback import LmmsEvalCallback
        callbacks.append(LmmsEvalCallback(tasks=training_args.auto_eval_tasks))
    ...
    if training_args.dpo:
        ...
        if training_args.use_one_logger:
            ...
            trainer = newLLaVATrainer(model=model, processing_class=tokenizer, args=training_args, callbacks=callbacks, **data_module)
        else:
            trainer = LLaVATrainer(model=model, processing_class=tokenizer, args=training_args, callbacks=callbacks, **data_module)

    logger.info(
        f"Dataloader: batch {len(trainer.get_train_dataloader())}, total samples {len(trainer.train_dataset)}"
    )
    # logger.info(
    #     f"[GPU memory] before trainer: {torch.cuda.memory_allocated() / (1024 ** 3)} GB")
    ...
    if training_args.lora_enable:
        if training_args.use_one_logger:
            one_logger_callback_utils.on_save_checkpoint_start()
        ...
        if training_args.use_one_logger:
            one_logger_callback_utils.on_save_checkpoint_success()
            one_logger_callback_utils.on_save_checkpoint_end()
    else:
