...
from transformers.pytorch_utils import ALL_LAYERNORM_LAYERS  # moved out of transformers.trainer in v5
...
class LLaVATrainer(Trainer):
    ...
    def _get_train_sampler(self, train_dataset: Optional[torch.utils.data.Dataset] = None) -> Optional[torch.utils.data.Sampler]:
        # transformers v5 passes train_dataset explicitly; fall back to self.train_dataset for older paths.
    ...
    def log(self, logs: Dict[str, float], start_time: Optional[float] = None) -> None:  # v5 adds start_time param
        """
