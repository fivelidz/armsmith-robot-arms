...
def training_step(
    ...
    # transformers v5 dropped Trainer.use_apex (apex no longer supported).
    # Default to False so the non-apex path runs.
...
def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
    ...
    # Save past state if it exists (legacy TransformerXL-style; removed from
    # TrainingArguments in transformers v5). Default to -1 so the block becomes
    # a no-op when the attribute is absent — NVILA/Qwen don't use this path.
