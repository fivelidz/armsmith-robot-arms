...
from data.utils.som_helper import apply_time_marks
...
def _load_video_with_time(
    video_path: str, *, num_frames: int, paint: bool = False,
) -> tuple[list[PIL.Image.Image], list[float]]:
    """Sample frames + their wall-clock seconds. If `paint=True`, also stamp the
    seconds onto each frame via apply_time_marks (the `use_time_marks` feature).
    TPE only needs the seconds, not the painting, so default is paint=False."""
    # Load video frames from a directory
    if os.path.isdir(video_path):
        frame_paths = sorted(glob.glob(os.path.join(video_path, "*")))
        indices = np.round(np.linspace(0, len(frame_paths) - 1, num_frames)).astype(int)
    ...
    for index in indices:
        ...
        sample_sec = (vidcap.get(cv2.CAP_PROP_POS_MSEC) / 1000.0)
        if paint:
            frame = apply_time_marks(frame, sample_sec)
        sample_secs[index] = sample_sec

        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        frames[index] = PIL.Image.fromarray(frame)

    sampled_frames = [frames[index] for index in indices if index in frames]
    return sampled_frames, [sample_secs[index] for index in indices if index in sample_secs]
...
def _extract_video(video: Video, config: PretrainedConfig) -> List[PIL.Image.Image]:
    ...
    use_time_marks = getattr(config, "use_time_marks", False)
    use_time_pe = getattr(config, "use_time_pe", False)
    if use_time_marks or use_time_pe:
        frames, sampled_secs = _load_video_with_time(
            video.path, num_frames=num_frames, paint=use_time_marks,
        )
        return frames, sampled_secs
        ...
        for part in make_list(message["value"]):
            if isinstance(part, str):
                ...
                if draft:
                    ...
                    need_secs = (
                        getattr(config, "use_time_marks", False)
                        or getattr(config, "use_time_pe", False)
                    )
                    if need_secs:
                        video, sampled_sec = _extract_video(part, config)
                        media["video"].append(video)
                        media["sampled_sec"].append(sampled_sec)
                    else:
