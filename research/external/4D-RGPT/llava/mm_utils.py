...
def tokenizer_image_token(prompt, tokenizer, image_token_index=None, return_tensors=None):
    """Tokenize a prompt that may contain `<image>` tokens.

    `image_token_index` is the legacy haotian-liu/LLaVA 3rd-positional arg
    (the sentinel id `<image>` is replaced with). Our tokenizer handles the
    image token via its own special-token vocabulary, so we accept and
    ignore it — kept for source-compat with callers like lmms-eval's `vila`
    adapter that still pass `IMAGE_TOKEN_INDEX` positionally.
    """
    del image_token_index  # unused; see docstring
