...
namespace ArmSmith
{
    ...
    public class Gripper : MonoBehaviour
    {
        ...
        {
            if (ab == null) return;
            // Do NOT push a drive target onto a body whose X axis is LOCKED. The fixed/reference jaw is a
            // static, all-axes-locked prismatic body; writing xDrive.target on a locked DOF makes the PhysX
            // articulation solver diverge — the body drifts to absurd positions (observed: the fixed jaw
            // flying to y=-15m and the gripper "exploding"). This was the real "holding physics seems wrong"
            // bug. Only drive bodies with a free (limited) X DOF.
            if (ab.linearLockX == ArticulationDofLock.LockedMotion) return;
            var d = ab.xDrive;
            d.target = target;
            ab.xDrive = d;
        }
